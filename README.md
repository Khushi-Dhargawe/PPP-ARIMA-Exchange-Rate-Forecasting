...
This isn't just an academic exercise — the finding that no model beats a random walk has direct implications 
for how businesses hedge FX risk, structure international contracts, and evaluate cross-border investments.
...
|---|---|---|
| Engle-Granger ADF on residuals | p = 0.61 → **No cointegration → Absolute PPP rejected** | Cannot use price levels to predict GBP/JPY — hedge accordingly |
| Relative PPP slope | β ≈ −1.42, p > 0.05, R² ≈ 0.01 → **Relative PPP rejected** | Inflation targeting alone won't protect FX exposure |
...
With intraday tick data, I'd test whether PPP holds at higher frequencies or only breaks at monthly aggregation. 
With options market data, I'd compare ARIMA forecast intervals against implied volatility — a real-world calibration check that treasury desks actually use.
...
```bash
git clone https://github.com/Khushi-Dhargawe/PPP-ARIMA-Exchange-Rate-Forecasting.git
